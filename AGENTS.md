...
- `packages/fonts`, `packages/email`, `packages/import`, `packages/utils`, and `packages/config` provide focused support surfaces. Prefer their existing exports over adding cross-package shortcuts.
...
- Server-owned HTTP behavior lives in `apps/server/src/{http,rpc,openapi,static,startup}`. Keep API/RPC/auth/static route wiring in `apps/server`, not in web routes.
...
- `turbo boundaries` is the executable package-boundary check. Workspace-level `turbo.json` files declare coarse tags:
  ...
  - `runtime:server` for server-only packages such as API/auth/db/env/email.
...
2. If the change is a server HTTP route/adapter, startup check, static handler, or OpenAPI/well-known handler, start in `apps/server/src`.
...
7. If it is a generic UI primitive or hook, put it in `packages/ui`; if it is workflow-specific UI, keep it in the owning web feature.
8. If it is a narrow cross-cutting helper, add an explicit `packages/utils` export only after checking that no domain package is a better owner.

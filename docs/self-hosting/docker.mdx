...
description: "A comprehensive guide to self-host HeadCV with Docker (Postgres only), including a detailed environment variable reference and troubleshooting tips."
---
...
HeadCV can be self-hosted using Docker in a matter of minutes, and this guide will walk you through the process. Here are some of the services you'll need to get started:
...
- Docker Hub: `AHMED9937/headcv:latest`
- GitHub Container Registry: `ghcr.io/AHMED9937/headcv:latest`
...
Create a new folder (for example `headcv/`) with:
...
SMTP_USER=""
SMTP_PASS=""
SMTP_FROM="HeadCV <noreply@headcv.com>"
SMTP_SECURE="false"
...
# The rest of HeadCV can run without these.
REDIS_URL=""
# Generated using `openssl rand -hex 32`
ENCRYPTION_SECRET=""
...
```
  ...
  <Step title="Create compose.yml">
    This setup runs Postgres and HeadCV on a private Docker network.
...
services:
  ...
  headcv:
    image: AHMED9937/headcv:latest
    # image: ghcr.io/AHMED9937/headcv:latest
    restart: unless-stopped
    ports:
      - "3000:3000"
    env_file:
...
```
    ...
    <Tip>
      Prefer pulling from Docker Hub? Keep <code>AHMED9937/headcv:latest</code>. Prefer GHCR? Swap it to <code>ghcr.io/AHMED9937/headcv:latest</code>.
    </Tip>

    <Note>
      In Docker, the HeadCV server listens on <code>PORT</code> and serves both the API and the built web app.
...
```bash
docker compose logs -f headcv
```

    </CodeGroup>

    HeadCV should now be available at your `APP_URL` (for the example above: `http://localhost:3000`).
...
```
  ...
  <Accordion title="Email (SMTP, optional)">
    ...
    - **`SMTP_FROM`**: Default from address (for example, `HeadCV <noreply@headcv.com>`).
...
<Accordion title="AI features (optional)">
  Saved AI provider management is usable only when **`ENCRYPTION_SECRET`** is configured. The AI Agent workspace also requires **`REDIS_URL`**. The rest of HeadCV can run without them.
...
To update your HeadCV installation to the latest available version, follow these steps:
...
Regular backups are essential to protect your data. HeadCV stores data in two places: the PostgreSQL database and file uploads (either local storage or S3).
...
HeadCV exposes a health check endpoint at `/api/health` that verifies the application and its dependencies. It checks **database** and **storage**; if either is unhealthy, the endpoint returns HTTP `503`.
...
This is particularly useful in high-availability setups where you have multiple instances of HeadCV. If one instance becomes unhealthy (for example, it loses database or storage connectivity), the reverse proxy will stop routing traffic to it until it recovers.
...
You can manually verify the health of your HeadCV instance:

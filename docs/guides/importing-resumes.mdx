...
description: "Learn which file formats HeadCV can import and how to import an existing resume from the dashboard."
---

HeadCV can create a new resume from several existing file formats. Use import when you are moving from another tool, restoring a backup, or converting an older HeadCV file.

## Supported import formats

| Format | Requires AI integration? | Notes |
| --- | --- | --- |
| **HeadCV (JSON)** | No | Best option for backups exported from the current version of HeadCV. |
| **HeadCV v4 (JSON)** | No | Use this for files exported from HeadCV v4. |
| **JSON Resume** | No | Use this for files that follow the JSON Resume schema. |
| **PDF** | Yes | HeadCV asks your configured AI provider to parse the file into structured resume data. |
| **Microsoft Word** | Yes | Supports Word documents. HeadCV asks your configured AI provider to parse the document. |
...
<Steps>
  ...
  <Step title="Import">
    Click **Import**. HeadCV creates a new resume and opens it in the builder when the import succeeds.
  </Step>
</Steps>

## Choose the right import type

If you have a file exported from HeadCV, choose **HeadCV (JSON)**. This preserves the most information because the file already matches HeadCV's data model.

If you are coming from HeadCV v4, choose **HeadCV v4 (JSON)**.
...
Make sure you selected the correct JSON import type. A HeadCV JSON export, a HeadCV v4 JSON export, and a JSON Resume file are different formats.

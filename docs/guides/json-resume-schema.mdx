...
description: "Learn about the HeadCV JSON Schema, available at /schema.json, and how it aims to become the gold standard for portable resume data."
---
...
HeadCV aims to be the **gold standard in JSON Resume Schemas**. A universal, well-documented, and rigorously typed schema that any service can adopt to enable seamless resume portability across the entire job application ecosystem.

Imagine:

- Exporting your resume from HeadCV as a JSON file
...
The HeadCV JSON Schema is publicly available at:

<Info>[https://headcv.com/schema.json](https://headcv.com/schema.json)</Info>

This endpoint returns a complete JSON Schema (draft 07 compatible) that describes the structure of a HeadCV document. It includes:
...
```json
{
  "$schema": "https://headcv.com/schema.json",
  "version": "5.0.0"
}
```

When importing resumes, HeadCV uses this version to handle migrations and ensure backward compatibility.
...
const ajv = new Ajv();
const schema = await fetch("https://headcv.com/schema.json").then((r) => r.json());
...
- **Job portals** can accept HeadCV JSON files directly
...
Below is the complete JSON Schema for HeadCV. You can also fetch the latest version directly from [https://headcv.com/schema.json](https://headcv.com/schema.json).

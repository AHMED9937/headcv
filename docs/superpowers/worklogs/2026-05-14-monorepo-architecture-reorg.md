...
- Task 1 completed:
  - Created `packages/resume` as `@headcv/resume`.
  ...
  - Updated all old `@headcv/utils/resume/patch` and `@headcv/utils/network-icons` imports to `@headcv/resume/patch` and `@headcv/resume/icons`.
  ...
  - Did not remove `@headcv/schema` from `@headcv/utils` because DOCX files still import schema and Task 2 owns that extraction.
- Validation for Task 1:
  - Initial `pnpm --filter @headcv/resume test` failed before Vitest because pnpm wanted to refresh modules in a non-TTY.
  ...
  - `pnpm --filter @headcv/resume test` passed: 2 files, 43 tests.
  - `pnpm --filter @headcv/resume typecheck` passed.
  - `pnpm --filter @headcv/ai typecheck` passed.
  - `pnpm --filter @headcv/api typecheck` passed.
  - `pnpm --filter web typecheck` passed.
  - Additional direct-consumer checks passed: `pnpm --filter @headcv/db typecheck`, `pnpm --filter @headcv/import typecheck`, and `pnpm --filter @headcv/utils typecheck`.
...
- Task 1 review correction:
  - Spec review found `packages/db` had a type-only manifest dependency on `@headcv/resume`.
  ...
  - Removed `@headcv/resume` from `packages/db/package.json` and `pnpm-lock.yaml`.
- Task 1 review validation:
  - `pnpm --filter @headcv/db typecheck` passed after the DB boundary correction.
  - `pnpm --filter @headcv/api typecheck` passed.
  - `pnpm --filter web typecheck` passed.
  - `pnpm --filter @headcv/resume test` passed: 2 files, 43 tests.
  - `pnpm --filter @headcv/resume typecheck` passed.
...
- Task 2 completed:
  - Created `packages/docx` as `@headcv/docx` with source-consumed root export.
  - Moved DOCX implementation/tests from `packages/utils/src/resume/docx` into `packages/docx/src`.
  - Updated web DOCX callers to import `buildDocx` from `@headcv/docx`.
  - Removed the old `@headcv/utils/resume/docx` export.
  - Removed `docx` and `@headcv/schema` from `@headcv/utils`.
  - Added `@headcv/docx` as a web dependency.
- Task 2 validation:
  - `pnpm --filter @headcv/docx test` passed: 5 files, 46 tests.
  - `pnpm --filter @headcv/docx typecheck` passed.
  - `pnpm --filter @headcv/utils typecheck` passed.
  - `pnpm --filter web typecheck` passed.
  - Spec review found only stale coordination text; plan and handoff were corrected.
- Task 2 completed:
  - Created `packages/docx` as `@headcv/docx` with source-consumed root export `"." -> "./src/index.ts"`.
  - Moved `packages/utils/src/resume/docx/*` to `packages/docx/src/*`, keeping the implementation and tests colocated.
  - Updated builder DOCX export callers and the export-section test mock from `@headcv/utils/resume/docx` to `@headcv/docx`.
  - Removed the old `@headcv/utils` `./resume/docx` export.
  - Removed `docx` and `@headcv/schema` from `@headcv/utils`; the new DOCX package owns those dependencies and depends on `@headcv/utils` only for shared color parsing.
  - Added `@headcv/docx` as a web dependency and refreshed `pnpm-lock.yaml`.
- Validation for Task 2:
  - Initial `pnpm --filter @headcv/docx test` failed on pnpm's non-TTY dependency-status install guard.
  - `CI=true pnpm --filter @headcv/docx test` then failed because sandbox DNS blocked registry fetches while pnpm recreated `node_modules`.
  - Retried `CI=true pnpm --filter @headcv/docx test` with network approval; passed: 5 files, 46 tests.
  - `pnpm --filter @headcv/docx typecheck` passed.
  - `pnpm --filter @headcv/utils typecheck` passed.
  - `pnpm --filter web typecheck` passed.
...
- Task 3 in progress:
  ...
  - Added `@headcv/pdf/browser` and `@headcv/pdf/server` package exports.
  - Kept Lingui locale loading in `apps/web/src/libs/resume/pdf-document.tsx`; the web wrapper now resolves localized section titles and delegates blob generation to `@headcv/pdf/browser`.
  - Updated `apps/web/src/libs/resume/pdf-document.server.tsx` to delegate file generation to `@headcv/pdf/server` after resolving localized section titles.
  ...
  - Updated `apps/server/src/handlers/resume-pdf.tsx` to reuse `@headcv/pdf/server` and preserve the existing `File` response body flow.
  - Added package helper tests for browser/server generation adapters.
- Task 3 validation so far:
  - Red test: `pnpm --filter @headcv/pdf test -- src/browser.test.tsx src/server.test.tsx` failed because `packages/pdf/src/browser.tsx` and `packages/pdf/src/server.tsx` did not exist.
  ...
  - `pnpm --filter @headcv/pdf test -- src/browser.test.tsx src/server.test.tsx` passed; Vitest reported 17 files and 139 tests.
...
- Task 4 completed:
  - Created `packages/mcp` as `@headcv/mcp` with source-consumed exports for the compact public surface and direct server-card/tool/prompt/resource subpaths.
  ...
  - Updated `apps/server/src/handlers/mcp.ts` and `apps/server/src/handlers/metadata.ts` to import from `@headcv/mcp`, removing the app-to-app source imports from `apps/server` into `apps/web`.
  ...
  - Updated prompts, server instructions, server-card metadata, and tests to use the canonical names and removed `headcv_*` aliases/instructions.
  ...
  - Updated package manifests and `pnpm-lock.yaml`; `web` no longer owns `@modelcontextprotocol/sdk`, and `server` now depends on `@headcv/mcp`.
- Validation for Task 4:
  - Initial `pnpm --filter @headcv/mcp test` reported no matching package before creation.
  - After package creation, `pnpm --filter @headcv/mcp test` hit pnpm's non-TTY dependency-status install guard.
  - `CI=true pnpm --filter @headcv/mcp test` then failed with a frozen-lockfile mismatch after moving dependencies.
  - Ran `pnpm install --lockfile-only --no-frozen-lockfile`; completed.
  - A later `CI=true pnpm --filter @headcv/mcp test` failed because sandbox DNS blocked registry fetches while pnpm restored `node_modules`.
  - Retried `CI=true pnpm --filter @headcv/mcp test` with network approval; passed: 4 files, 29 tests.
  - `pnpm --filter @headcv/mcp typecheck` initially failed because the package type graph reaches API/auth TSX and tests had optional text access; fixed by enabling JSX in the package tsconfig and narrowing test content access.
  - Final validation commands passed: `pnpm --filter @headcv/mcp test`, `pnpm --filter @headcv/mcp typecheck`, `pnpm --filter server typecheck`, `pnpm --filter @headcv/ai typecheck`, focused `pnpm exec biome check ...`, and `rg -n "\\.\\./\\.\\./\\.\\./web/src|apps/web/src|web/src/routes/mcp" apps/server/src packages/mcp/src`.
  ...
  - `pnpm --filter @headcv/pdf test` passed: 17 files, 139 tests.
  - `pnpm --filter @headcv/pdf typecheck` passed.
  - `pnpm --filter web typecheck` passed.
...
- Task 4 review correction:
  ...
  - Re-ran `rg "headcv_|tailor_resume|web/src/routes/mcp|apps/web/src|\\.\\./\\.\\./\\.\\./web" apps/server packages/mcp packages/ai/src docs/guides/using-the-mcp-server.mdx -n`; no matches.
  - Re-ran `pnpm --filter @headcv/mcp test`, `pnpm --filter @headcv/mcp typecheck`, `pnpm --filter server typecheck`, `pnpm --filter @headcv/ai typecheck`, and focused `pnpm exec biome check ...`; all passed.
- Task 5 review correction:
  ...
  - Re-ran `pnpm --filter @headcv/api test`: 19 files, 155 tests passed.
  - Re-ran `pnpm --filter @headcv/api typecheck`, `pnpm --filter server typecheck`, and `pnpm --filter web typecheck`; all passed.
...
- Task 5 completed:
  ...
  - Moved the authenticated PDF download procedure from `apps/server/src/handlers/resume-pdf.tsx` into `packages/api/src/features/resume/export.ts`; it now calls `@headcv/pdf/server`.
  ...
  - Updated `apps/web` to keep API imports type-only, including the `FeatureFlags` type from `@headcv/api/features/flags`.
  - Added `@headcv/pdf` as an API package dependency and refreshed `pnpm-lock.yaml`.
- Validation for Task 5:
  - Initial `pnpm --filter @headcv/api typecheck` failed because pnpm attempted a non-TTY dependency-status install after package metadata changes.
  - Retried with `CI=true`; sandbox DNS blocked registry fetches while rebuilding `node_modules`.
  - Retried `CI=true pnpm --filter @headcv/api typecheck` with network approval; dependency restoration completed and the first compiler pass found moved import paths that needed correction.
  - `pnpm --filter @headcv/api test` initially failed because the moved agent service test mocked the new `./service` module instead of its relocated AI/resume dependencies; fixed the mocks.
  - Final focused validations passed: `pnpm --filter @headcv/api test`, `pnpm --filter @headcv/api typecheck`, `pnpm --filter server typecheck`, `pnpm --filter web typecheck`, and `pnpm --filter @headcv/mcp typecheck`.
  ...
  - Repo scans passed for no remaining `@headcv/api/services/*` or `@headcv/api/helpers/*` imports outside `packages/api`, no internal `../services` or `../helpers` imports, and no `./services/*` or `./helpers/*` wildcard exports in `packages/api/package.json`.
- Task 6 completed:
  ...
  - Preserved explicit API runtime imports only: `@headcv/api/routers`, `@headcv/api/features/storage`, and `@headcv/api/features/resume/export`.
- Validation for Task 6:
  ...
  - Final `pnpm --filter @headcv/api typecheck` passed.
  ...
  - `rg -n "apps/web/src|from ['\"][^'\"]*web/src|@headcv/api/services/" apps/server/src` returned no matches.
  - `rg -n "@headcv/api/services/" apps/server packages/api apps/web packages` returned no matches.
...
- Task 7 slice 1 completed:
  ...
  - Moved web-local PDF document wrappers from `apps/web/src/libs/resume/pdf-document*.tsx` to `apps/web/src/features/resume/export/pdf-document*.tsx`; the wrappers still resolve localized section titles in web and call `@headcv/pdf/browser` or `@headcv/pdf/server`.
...
- Validation for Task 8 so far:
  ...
  - `rg -n "@headcv/schema/resume/data|awardItemSchema|certificationItemSchema|coverLetterItemSchema|customSectionSchema|educationItemSchema|experienceItemSchema|interestItemSchema|languageItemSchema|profileItemSchema|projectItemSchema|publicationItemSchema|referenceItemSchema|skillItemSchema|summaryItemSchema|volunteerItemSchema|z\\.object\\(\\{ type" apps/web/src/dialogs/store.ts` returned no matches.
- Task 9 completed:
  ...
  - Removed the cross-workspace `@headcv/ui/* -> ../../packages/ui/src/*` path alias from `apps/web/tsconfig.json`; web now relies on the UI package export map.

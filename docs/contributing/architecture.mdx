...
description: "Understand the HeadCV monorepo, runtime boundaries, and package ownership model"
---

HeadCV is a pnpm/Turborepo monorepo. The product runs as one deployed Node.js process, while the source is split into a full-stack web app, a server adapter, and focused internal packages.
...
> This fork (HeadCV v1) removes all AI and MCP features from the upstream project. `packages/ai`, `packages/mcp`, and the `/mcp` route do not exist in this codebase; the original code is preserved in the `archive/upstream-ai-mcp-v5.1.6` branch.
...
`apps/web` owns the TanStack Start experience. `apps/server` owns the production Hono process and mounts RPC, auth, OpenAPI, static uploads, schema JSON, and the built web app.
...
| `apps/server` | Hono route composition, production HTTP adapters, OpenAPI/well-known handlers, static file serving, startup checks |
...
- Use `@headcv/*` package exports for cross-workspace imports.
- Do not import another workspace through `apps/**`, `packages/**`, `@headcv/*/src/**`, or a TypeScript path alias to another workspace's `src`.
...
- `@headcv/pdf/browser` creates browser PDF blobs.
- `@headcv/pdf/server` creates server PDF files.

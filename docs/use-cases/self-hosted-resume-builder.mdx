...
description: "Run HeadCV as a self-hosted resume builder with Docker, PostgreSQL, optional storage, SSO, and migration guidance."
---

HeadCV can be run as a self-hosted resume builder when you want to operate the app on your own infrastructure instead of using the hosted instance.
...
Self-hosted deployments can also expose API endpoints from their own domain.

For automation setup on self-hosted instances, see [Using the API](/guides/using-the-api).

<Tip>
  Replace hosted URLs such as <code>https://headcv.com</code> with your own instance URL when following API, MCP, or
  sharing examples.
</Tip>

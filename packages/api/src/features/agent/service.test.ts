...
vi.mock("./streams", () => ({
	agentStreamLifecycle: { create: vi.fn(), resume: vi.fn(), ensureAvailable: vi.fn() },
}));
vi.mock("./tools", () => ({
	buildAgentInstructions: vi.fn(),
	buildAgentTools: vi.fn(() => ({})),
}));
vi.mock("@headcv/utils/string", () => ({ generateId: () => "test-id" }));
...
function buildActionRecord(overrides: Record<string, unknown> = {}) {
	return {
		id: "action-1",
		userId: "user-1",
		threadId: "thread-1",
...
describe("agentService.messages.send", () => {
	...
	it("checks streaming availability before saving a message or starting a model run", async () => {
		dbMock.select.mockImplementationOnce(() => selectLimitResult([buildActiveThread()]));
		const { agentStreamLifecycle } = await import("./streams");
		vi.mocked(agentStreamLifecycle.ensureAvailable).mockRejectedValueOnce(new ORPCError("SERVICE_UNAVAILABLE"));
		const { agentService } = await import("./service");
		...
		await expect(
			agentService.messages.send({
				threadId: "thread-1",
				userId: "user-1",
				message: { id: "message-1", role: "user", parts: [{ type: "text", text: "Hello" }] },
			}),
	...
	it("stores snapshotData and applies a valid JSON Patch without a timestamp conflict guard", async () => {
		...
		dbMock.select
			.mockImplementationOnce(() => selectForUpdateResult([{ data: beforeData, updatedAt: beforeUpdatedAt }]))
			.mockImplementationOnce(() => selectLimitResult([]));
...
describe("agentService.actions.revert", () => {
	...
	it("propagates RESUME_VERSION_CONFLICT when snapshot restore detects a newer resume", async () => {
		const action = buildAction();
		...
		await expect(agentService.actions.revert({ id: "action-1", userId: "user-1" })).rejects.toMatchObject({
...
describe("agentService.actions.latestByResume", () => {
	function buildAction(overrides: Record<string, unknown> = {}) {
		return {
			id: "action-1",
			userId: "user-1",
			...
			operations: [{ op: "replace", path: "/summary/content", value: "New summary" }],
			snapshotData: { summary: { content: "Old summary" } },
			baseUpdatedAt: new Date("2026-05-01T00:00:00.000Z"),
			appliedUpdatedAt: new Date("2026-05-02T00:00:00.000Z"),
			revertedAt: null,
			revertMessage: null,
	...
	it("returns the latest applied resume patch with snapshotData", async () => {
		const action = buildAction();
		dbMock.select.mockImplementation(() => selectWhereOrderByLimitResult([action]));

		const { agentService } = await import("./service");

		const result = await agentService.actions.latestByResume({ resumeId: "resume-1", userId: "user-1" });
	...
	it("returns null when no applied resume patch exists", async () => {
		dbMock.select.mockImplementation(() => selectWhereOrderByLimitResult([]));
...
describe("agentService.actions.keep", () => {
	it("returns an already accepted action without changing the resume or action", async () => {
		dbMock.select.mockImplementation(() => selectLimitResult([buildActionRecord({ status: "accepted" })]));
		const { agentService } = await import("./service");

		await expect(agentService.actions.keep({ id: "action-1", userId: "user-1" })).resolves.toMatchObject({
			status: "accepted",
			canRollback: false,
		});
		expect(dbMock.update).not.toHaveBeenCalled();
	...
	it("rejects an unavailable action without writing", async () => {
		dbMock.select.mockImplementation(() => selectLimitResult([]));
		const { agentService } = await import("./service");

		await expect(agentService.actions.keep({ id: "missing", userId: "user-1" })).rejects.toMatchObject({
			code: "NOT_FOUND",
		});
		expect(dbMock.update).not.toHaveBeenCalled();
	});

	it("accepts the pending resume patch action", async () => {
		const action = buildActionRecord();
		const acceptedAction = buildActionRecord({ status: "accepted" });
		dbMock.select.mockImplementation(() => selectLimitResult([action]));
		dbMock.update.mockReturnValue({
			set: vi.fn(() => ({
				where: vi.fn(() => ({ returning: vi.fn(async () => [acceptedAction]) })),
			})),
		});

		const { agentService } = await import("./service");
		const result = await agentService.actions.keep({ id: "action-1", userId: "user-1" });

		expect(result.status).toBe("accepted");
		expect(dbMock.update).toHaveBeenCalledOnce();
	});

	it("rejects keeping a non-patch action", async () => {
		dbMock.select.mockImplementation(() => selectLimitResult([buildActionRecord({ kind: "analysis" })]));

		const { agentService } = await import("./service");

		await expect(agentService.actions.keep({ id: "action-1", userId: "user-1" })).rejects.toMatchObject({
			code: "BAD_REQUEST",
		});
		expect(dbMock.update).not.toHaveBeenCalled();
	});
...
describe("applyResumePatch pending-action guard", () => {
	it("rejects a second patch when an applied action already exists for the resume", async () => {
		const activeThread = buildActiveThread();
		const persistedMessage = {
			id: "message-1",
			userId: "user-1",
			...
			uiMessage: {
				id: "ui-message-1",
				role: "user",
				parts: [{ type: "text", text: "Add a skill" }],
			},
		};
		...
		vi.mocked(convertToModelMessages).mockResolvedValue([
			{ role: "user", content: [{ type: "text", text: "Add a skill" }] },
		]);
		class MockToolLoopAgent {
			stream = vi.fn(async () => ({ toUIMessageStream: vi.fn(() => new ReadableStream()) }));
		}
		...
		const beforeData = { basics: { name: "Alice" } };
		const beforeUpdatedAt = new Date("2026-05-01T00:00:00.000Z");

		// SELECT … FOR UPDATE returns the resume row, then the pending-action check returns a row
		dbMock.select
			.mockImplementationOnce(() => selectForUpdateResult([{ data: beforeData, updatedAt: beforeUpdatedAt }]))
			.mockImplementationOnce(() => selectLimitResult([{ id: "existing-action" }]));

		const toolConfig = vi.mocked(buildAgentTools).mock.calls.at(-1)?.[0];
		// biome-ignore lint/suspicious/noExplicitAny: captured mocked tool config has intentionally loose handler types
		const applying = (toolConfig as any).handlers.applyResumePatch({
			title: "Add skill",
			operations: [{ op: "add", path: "/sections/skills/items/-", value: { id: "s-1", name: "React" } }],
		});

		await expect(applying).rejects.toBeInstanceOf(ORPCError);

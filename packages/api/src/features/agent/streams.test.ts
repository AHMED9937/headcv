...
vi.mock("@headcv/env/server", () => ({ env: { REDIS_URL: "redis://localhost:6379" } }));

vi.mock("@headcv/db/client", () => ({ db: { update: vi.fn() } }));
...
describe("agent stream lifecycle", () => {
	it("reports a recoverable service error when the connection check fails, then succeeds after recovery", async () => {
		const checkConnection = vi.fn().mockRejectedValueOnce(new Error("ECONNREFUSED")).mockResolvedValue("PONG");
		const getContext = vi.fn();
		const lifecycle = createAgentStreamLifecycle({ getContext, checkConnection });

		await expect(lifecycle.ensureAvailable()).rejects.toMatchObject({
			code: "SERVICE_UNAVAILABLE",
			message: expect.stringContaining("streaming service cannot connect"),
		});
		await expect(lifecycle.ensureAvailable()).resolves.toBeUndefined();
		expect(getContext).not.toHaveBeenCalled();
	});

	it("reports stream creation and reconnection failures without exposing Redis details", async () => {
		const lifecycle = createAgentStreamLifecycle({
			getContext: () => ({
				createNewResumableStream: vi.fn().mockRejectedValue(new Error("private connection details")),
				resumeExistingStream: vi.fn().mockRejectedValue(new Error("private connection details")),
			}),
		});
		const makeStream = vi.fn();
		for (const request of [() => lifecycle.create("stream-1", makeStream), () => lifecycle.resume("stream-1")]) {

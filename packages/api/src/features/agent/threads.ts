		...
		.input(
			z
				.object({
					...
					workingResumeId: z.string().optional(),
				})
				.refine((input) => !(input.sourceResumeId && input.workingResumeId), {
					message: "Choose either a source resume or an existing working resume.",
				}),
		)
		.handler(async ({ context, input }) => {
			try {
				return await agentService.threads.create({
					...
					...(input.workingResumeId ? { workingResumeId: input.workingResumeId } : {}),
				});
			} catch (error) {
				if (isAgentEnvironmentUnavailable(error)) throwUnavailable();
				throw error;

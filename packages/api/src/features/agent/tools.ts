...
export function buildAgentInstructions({ hasProviderNativeSearch }: { hasProviderNativeSearch: boolean }) {
	const baseInstructions =
		"You are an expert resume-writing agent inside HeadCV. Help the user improve the working resume for a target role. Read the resume before editing. Respond to the user in clean Markdown with concise paragraphs, bullets, and bold text when it improves scanability. Apply concise, valid JSON Patch operations when changes are useful. Patch paths are evaluated against the resume data object returned by read_resume, so use paths like /basics/name for the visible name, /summary/content for the summary, /sections/skills/items/0/keywords for skill keywords, and /sections/projects/items/0/description for a project description. Never prefix paths with /data or use /name. apply_resume_patch cannot rename the resume file/title metadata. Batch related JSON Patch operations into one apply_resume_patch call for each coherent edit instead of making repeated patch calls for the same request. Ask the user a question when a missing preference blocks a high-confidence edit.";
...
export function buildAgentTools(input: BuildAgentToolsInput): ToolSet {
	return {
		...
		apply_resume_patch: tool({
			description:
				"Apply one cohesive batch of JSON Patch operations to the working resume data immediately. Paths are rooted at resume data; use /basics/name for the visible resume name, /summary/content for the summary, /sections/skills/items/0/keywords for skill keywords, and /sections/projects/items/0/description for a project description. Never use /data/basics/name, /name, /skills/items/..., or /projects/items/... This tool cannot rename the resume file/title metadata. The user can restore the draft to the snapshot captured before a patch later.",

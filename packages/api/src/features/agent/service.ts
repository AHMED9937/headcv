...
import { normalizeResumePatchOperationPaths } from "@headcv/ai/tools/patch-proposal";
...
function toActionWithSnapshot(row: AgentActionRecord) {
	return {
		...toAction(row),
...
async function createWorkingResume(input: CreateThreadInput) {
	if (input.sourceResumeId && input.workingResumeId) {
		throw new ORPCError("BAD_REQUEST", { message: "Choose either a source resume or an existing working resume." });
	}

	if (input.workingResumeId) {
		const resume = await resumeService.getById({ id: input.workingResumeId, userId: input.userId });
		return { id: resume.id, source: null, title: resume.name };
	}
...
async function applyResumePatch(input: {
	...
	const operations = normalizeResumePatchOperationPaths(input.operations);

	const { action, patched } = await db.transaction(async (tx) => {
		const [before] = await tx
			.select({ data: schema.resume.data, updatedAt: schema.resume.updatedAt })
			.from(schema.resume)
		...
		const [pendingAction] = await tx
			.select({ id: schema.agentAction.id })
			.from(schema.agentAction)
			.where(
				and(
					eq(schema.agentAction.userId, input.userId),
					eq(schema.agentAction.resumeId, input.resumeId),
		...
		if (pendingAction) {
			throw new ORPCError("PRECONDITION_FAILED", {
				message: "Keep or revert the pending AI review before applying another update.",
			});
		}
			...
			readResume: async () => {
				...
				return {
					...
					patchPathExamples: {
						...
						skillKeywords: "/sections/skills/items/0/keywords",
						projectDescription: "/sections/projects/items/0/description",
					},
					patchNotes: [
						"apply_resume_patch paths are rooted at the `data` object below.",
						"Do not prefix paths with `/data`.",
						"Section items live under `/sections/<family>/items/<index>/...` (e.g. /sections/skills/items/0/keywords).",
...
export const agentService = {
	threads: {
		...
		create: async (input: CreateThreadInput) => {
			...
			const selectedProvider = await aiProvidersService.getDefaultRunnable({ userId: input.userId });
	...
	messages: {
		send: async (input: SendMessageInput) => {
			...
			await agentStreamLifecycle.ensureAvailable();
	...
	actions: {
		latestByResume: async (input: { resumeId: string; userId: string }) => {
			assertAgentEnvironment();
			...
			return action ? toActionWithSnapshot(action) : null;
		},

		keep: async (input: { id: string; userId: string }) => {
			assertAgentEnvironment();
			...
			if (action.kind !== "resume_patch") throw new ORPCError("BAD_REQUEST");
			if (action.status !== "applied") return toAction(action);

			const [updated] = await db
				.update(schema.agentAction)
				.set({ status: "accepted", revertMessage: null })
				.where(
					and(
						eq(schema.agentAction.id, action.id),
						eq(schema.agentAction.userId, input.userId),
						eq(schema.agentAction.status, "applied"),
					),
				)
				.returning();
			return toAction(updated ?? action);
		},

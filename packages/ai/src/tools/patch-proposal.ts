...
import z from "zod";
import { applyResumePatches, jsonPatchOperationSchema } from "@headcv/resume/patch";

const RESUME_SECTION_FAMILIES = new Set([
	"profiles",
	"experience",
	"education",
	"projects",
...
function normalizePatchPath(path: string): string {
	...
	const [root, ...rest] = path.slice(1).split("/");
	if (!root || root === "sections" || !RESUME_SECTION_FAMILIES.has(root)) return path;

	return ["", "sections", root, ...rest].join("/");
}

export function normalizeResumePatchOperationPaths(operations: JsonPatchOperation[]): JsonPatchOperation[] {
	return operations.map((operation) => {
		const normalized: JsonPatchOperation = { ...operation, path: normalizePatchPath(operation.path) };

		if ("from" in operation && typeof operation.from === "string") {
			(normalized as Extract<JsonPatchOperation, { from: string }>).from = normalizePatchPath(operation.from);
		}
...
export function normalizeResumePatchProposals(
	...
	return input.proposals.map((proposal, index) => ({
		...
		operations: normalizeResumePatchOperationPaths(proposal.operations),

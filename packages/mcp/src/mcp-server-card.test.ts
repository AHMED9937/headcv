...
describe("buildMcpServerCard", () => {
	...
	it("identifies the server as headcv", () => {
		expect(card.serverInfo.name).toBe("headcv");
		expect(card.serverInfo.title).toBe("HeadCV");
		expect(card.serverInfo.websiteUrl).toBe("https://headcv.com");
	});

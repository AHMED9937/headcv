...
import z from "zod";
import { resumePatchOperationsSchema } from "@headcv/ai/tools/resume-tool-contracts";
...
export function buildMcpServerCard(appVersion: string) {
	const tools = [
		...
		},
		{
			...
			description: [
				...
				"Analyses are created from the HeadCV web app AI flow, not from MCP.",
		...
		},
		{
			...
			title: "Import Resume",
			description: [
				"Create a new resume from a full ResumeData JSON object (e.g. an exported file from HeadCV).",
	...
	return {
		...
		serverInfo: {
			...
			version: appVersion,
			title: "HeadCV",
			websiteUrl: "https://headcv.com",
			description:
				"HeadCV is a guided resume builder. Use this MCP server to interact with your resume using an LLM of your choice.",
			icons: [
				{ src: "https://headcv.com/icon/light.svg", mimeType: "image/svg+xml", theme: "light" as const },
				{ src: "https://headcv.com/icon/dark.svg", mimeType: "image/svg+xml", theme: "dark" as const },
			],
		},
		tools,
		prompts,

...
### RES-03: Best-practice research

**When:** You need to learn and apply the current best practices for a technology, pattern, or workflow.

**Prompt:**

```text
Act as a senior technical researcher focused on best practices.
Topic: [technology / pattern / workflow / library]
Research the current industry best practices for this topic.
For each practice or recommendation, report:
- what it is and when to apply it
- why it is recommended (problem it prevents or value it adds)
- concrete examples relevant to THIS project (Nx + pnpm + Next.js App Router + NestJS + Prisma + Neon stack)
- trade-offs or situations where it does NOT apply
- official source or authoritative reference
Summarize the top 5–7 practices as a checklist.
If a practice conflicts with an existing project decision, flag it and recommend how to resolve the conflict.
Record the findings in docs/pack/best-practices/[topic-slug].md.
```

**Output:** `docs/pack/best-practices/[topic-slug].md`

**Done when:** best-practice checklist is recorded and conflicts with existing decisions are flagged.

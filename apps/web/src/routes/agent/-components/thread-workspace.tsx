...
import { ResizableGroup, ResizablePanel, ResizableSeparator } from "@headcv/ui/components/resizable";
...
import { AgentThreadSidebar } from "./thread-sidebar";
		...
		<div className={cn("flex", isUser ? "justify-end" : "justify-start")}>
			...
			>
				{message.parts.map((part, index) => (
					<MessagePart
						key={"toolCallId" in part ? getMessagePartKey(message.id, part) : `${message.id}-${part.type}-${index}`}
						part={part}
						isUser={isUser}
						onAnswer={onAnswer}
						onRevert={onRevert}
...
export function AgentChat({
	...
	const refreshThread = useCallback(async () => {
		...
		if (resumeId) {
			invalidations.push(
				...
				queryClient.invalidateQueries({ queryKey: orpc.resume.getById.queryKey({ input: { id: resumeId } }) }),
			);
		}

		await Promise.all(invalidations);
	}, [queryClient, resumeId, threadId]);
	...
	const handleArchive = () => {
		archiveMutation.mutate(
			{ id: threadId },
			{
				...
				onError: (error) => {
					toast.error(getAgentErrorMessage(error, t`Failed to archive thread.`));
				},
			},
		);
	};
	...
	const handleDelete = async () => {
		...
		deleteMutation.mutate(
			{ id: threadId },
			{
				...
				onError: (error) => {
					toast.error(getAgentErrorMessage(error, t`Failed to delete thread.`));
				},
			},
		);
	};
	...
	}, [messages, onActionApplied, refreshThread]);
	...
	const uploadFiles = async (files: FileList | null) => {
		...
		try {
			...
			toast.error(getAgentErrorMessage(error, t`Failed to upload attachment.`));
		} finally {
	...
	const revertAction = (actionId: string) => {
		...
		revertMutation.mutate(
			{ id: actionId },
			{
				...
				onError: (error) => toast.error(getAgentErrorMessage(error, t`Could not restore this patch.`)),
			},
		);
	};
			...
			<div className="mx-auto flex max-w-3xl flex-col gap-4 p-4">
				...
				{isStreaming ? (
					<div className="flex justify-start">
						<div
							role="status"
							className="flex items-center gap-2 rounded-md bg-muted px-4 py-3 text-muted-foreground text-sm"
						>
							<CircleNotchIcon aria-hidden className="size-4 motion-safe:animate-spin" />
							<Trans>Working…</Trans>
						</div>
					</div>
				) : null}
...
const AGENT_PREVIEW_ZOOM_STORAGE_KEY = "headcv:agent-preview-zoom:v3";
...
export function AgentThreadWorkspace({ threadId, draft }: { threadId: string; draft?: string }) {
	const navigate = useNavigate();

	...
	const handleArchive = () => {
		archiveMutation.mutate(
			{ id: thread.id },
			{
				...
				onError: (error) => toast.error(getAgentErrorMessage(error, t`Failed to archive thread.`)),
			},
		);
	};
		...
		deleteMutation.mutate(
			{ id: thread.id },
			{
				onSuccess: async () => {
					await queryClient.invalidateQueries({ queryKey: orpc.agent.threads.list.queryKey() });
					if (activeThreadId === thread.id) {
						if (onActiveThreadDeleted) onActiveThreadDeleted();
						else void navigate({ to: "/agent" });
					}
				},
				onError: (error) => toast.error(getAgentErrorMessage(error, t`Failed to delete thread.`)),
			},
		);
	};
	...
	return (
		...
		>
			{onSelect ? (
				<button
					type="button"
					className="block w-full min-w-0 rounded-md px-3 py-2 pe-10 text-start text-sm outline-hidden ring-ring focus-visible:ring-2"
			...
			<ThreadActions thread={thread} activeThreadId={activeThreadId} onActiveThreadDeleted={onActiveThreadDeleted} />
		</div>
	);
}
...
export function AgentThreadList({
	...
	const { data: allThreads, isLoading } = useQuery(orpc.agent.threads.list.queryOptions());
	const threads = workingResumeId
		? allThreads?.filter((thread) => thread.workingResumeId === workingResumeId)
		: allThreads;
			...
			<div className="space-y-1 p-2">
				...
				{threads?.length === 0 ? (
					<div className="rounded-md border border-dashed p-3 text-muted-foreground text-sm">
						{workingResumeId ? <Trans>No chats for this resume yet.</Trans> : <Trans>No threads yet.</Trans>}
					</div>
				) : null}
...
export function AgentThreadSidebar({
	...
	return (
		<aside className={cn("flex h-full min-h-0 flex-col border-e bg-muted/30", className)}>
			...
			<AgentThreadList activeThreadId={activeThreadId} />
		</aside>
	);
}
